{{/*
hiroba-platform.checks — operator dependency checks.

Called from a consumer wrapper, these checks fail at template-render time if
a feature is enabled but the required Operator CRDs are not registered in the
cluster. Prevents silent failures where Custom Resources are submitted but no
controller is watching them.

When rendering offline (helm template / CI), pass --api-versions to simulate
...
{{- if and .Values.postgres.backup.enabled (not (.Capabilities.APIVersions.Has "garage.rajsingh.info/v1beta1")) }}
